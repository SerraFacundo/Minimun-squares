...
from ajustes import ajuste_potencial
...
def ajustar():
    ...
    a, b, bondad = ajuste_potencial(x, y)
    
    label_result = tk.Label(window, text=f"La ecuación de la curva es: y = {a} * x^{b}")
    ...
    y_line = a * x_line**b
    ax.plot(x_line, y_line, color='red', label='Ajuste Potencial')
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.legend()
